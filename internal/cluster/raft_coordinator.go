...
//
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ (nil pointer dereference в RegisterNode):
//   Ранее поле coord.fsm (*RaftFSM) и его state (*RaftClusterState) нигде
//   не инициализировались в NewRaftCoordinator. Первый же вызов
//   RegisterNode (или любого метода, читающего rc.fsm.state) приводил
//   к панике SIGSEGV по адресу 0x0 на rc.fsm.state.mu.Lock().
//
//   Также не инициализировались coord.logStore и coord.stableStore
//   (*InmemStore), которые используются в monitorLeadership через
//   rc.stableStore.Set(...).
//
//   Теперь в NewRaftCoordinator создаются:
//     - logStore   = NewInmemStore(...)
//     - stableStore= NewInmemStore(...)
//     - fsm        = &RaftFSM{state: &RaftClusterState{...}}
//   до ветвления singleNodeMode / setupClusterMode.
//
//   Дополнительно добавлены defensive-проверки на nil в публичных
//   методах (RegisterNode, RemoveNode, GetAllNodes, GetActiveNodes,
//   GetNodeByID, GetLeader, SendHeartbeat, UpdateNodeStatus,
...
func (rrm *ReplicaReadManager) updateReplicaStatus() {
    ...
    for _, node := range nodes {
        if rrm.coordinator.localNodeInfo == nil {
            continue
        }
...
func (lfm *LeaderFallbackManager) SubmitWrite(data []byte) error {
    ...
    if !lfm.fallbackMode.Load() && lfm.coordinator != nil && lfm.coordinator.IsLeader() {
        if lfm.coordinator.raft == nil {
...
//
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ранее coord.fsm не создавался в
// NewRaftCoordinator, что приводило к SIGSEGV (nil pointer dereference)
// при первом обращении к rc.fsm.state. Теперь FSM создаётся явно,
// до ветвления singleNodeMode / setupClusterMode.
func (rc *RaftCoordinator) initRaftFSM(replicationFactor int32) {
    now := time.Now().UnixMilli()
    ...
    rc.fsm = &RaftFSM{
        state: &RaftClusterState{
            ...
            Shards:            make(map[string]*RangeShard),
            CreatedAt:         now,
            UpdatedAt:         now,
        },
        logger:      rc.logger,
...
// initRaftStores создаёт in-memory хранилища для Raft (logStore, stableStore).
//
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: ранее coord.logStore и coord.stableStore
// не инициализировались, что приводило к nil pointer dereference
// в monitorLeadership (rc.stableStore.Set(...)).
func (rc *RaftCoordinator) initRaftStores(dataDir string) {
    if dataDir == "" {
        dataDir = "raft_data"
    }
...
func NewRaftCoordinator(cfg *config.Config, store *storage.Storage, logger *log.Logger) (*RaftCoordinator, error) {
    ...
    // КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ: инициализируем logStore, stableStore и FSM
    // ДО первого обращения к coord.fsm.state (в т.ч. в singleNodeMode ветке
    // и в setupClusterMode).
    coord.initRaftStores(cfg.Cluster.RaftDataDir)
    coord.initRaftFSM(int32(replicationFactor))
    ...
    if coord.singleNodeMode {
        ...
        // Добавляем локальный узел в FSM state, чтобы GetActiveNodes /
        // GetAllNodes / GetNodeByID работали корректно в single-node режиме.
        coord.fsm.state.mu.Lock()
...
func (rc *RaftCoordinator) GetActiveNodes() []*NodeInfo {
    ...
    if rc.fsm == nil || rc.fsm.state == nil {
        if rc.singleNodeMode && rc.localNodeInfo != nil {
            return []*NodeInfo{rc.localNodeInfo}
        }
        return nodes
    }
    ...
    for _, nodeInfo := range state.Nodes {
        if now-nodeInfo.LastSeen < 30000 && nodeInfo.Status == "active" {
            if rc.splitBrainDetector == nil || !rc.splitBrainDetector.IsQuarantined(nodeInfo.ID) {
...
func (rc *RaftCoordinator) UpdateNodeStatus(nodeID string, status NodeStatus) error {
    ...
    if rc.splitBrainDetector != nil && rc.splitBrainDetector.IsQuarantined(nodeID) {
        return fmt.Errorf("node %s is quarantined, cannot update status", nodeID)
    }

    if rc.fsm == nil || rc.fsm.state == nil {
        return fmt.Errorf("raft FSM is not initialized")
    }
...
func (rc *RaftCoordinator) GetClusterStatus() *ClusterStatus {
    ...
    if rc.splitBrainDetector != nil && rc.splitBrainDetector.Detect(rc.currentTerm.Load(), leaderID, len(nodes)) {
        health = "split_brain"
    }
...
func (rc *RaftCoordinator) RegisterNode(node *Node) error {
    ...
    if rc.splitBrainDetector != nil && rc.splitBrainDetector.IsQuarantined(node.ID) {
...
func (rc *RaftCoordinator) HandleStatusSync(leaderID string, term uint64, clusterSize int) {
    ...
    if rc.splitBrainDetector != nil && rc.splitBrainDetector.Detect(term, leaderID, clusterSize) {
        ...
        if currentState == raft.Leader && rc.localNodeInfo != nil && leaderID != rc.localNodeInfo.ID {
