...
// RateLimiterConfigHTTP содержит конфигурацию rate limiter'а
...
func NewHTTPServerWithTLS(
    ...
    // ИСПРАВЛЕНО: Prometheus endpoints (без аутентификации).
    mux.HandleFunc("/-/healthy", s.handlePromHealthy)
...
// handlePromHealthy — health-check для Prometheus/Grafana
func (s *HTTPServer) handlePromHealthy(w http.ResponseWriter, r *http.Request) {
...
func (s *HTTPServer) handlePromMetrics(w http.ResponseWriter, r *http.Request) {
    ...
    w.WriteHeader(http.StatusOK)
    if _, err := metrics.DefaultRegistry().WriteTo(w); err != nil && s.logger != nil {
        s.logger.Error(fmt.Sprintf("Failed to write metrics: %v", err))
    }
}
...
// handleMetricsEndpoint обрабатывает запрос метрик (JSON, без аутентификации).
// Оставлен для совместимости с Grafana JSON API datasource.
// Для Prometheus используйте /metrics.
