...
// =============================================================================
// КОНСТАНТЫ (удалены дублирующиеся, используются из других файлов)
// =============================================================================

// VisibilityMapSize, MaxVersionsPerDoc, VersionRetentionDays 
// определены в trigger.go и используются в этом файле
...
// =============================================================================
// ФУНКЦИИ ДЛЯ КРОСС-ДАТАЦЕНТРОВОЙ МИГРАЦИИ
// =============================================================================

// GetDocumentAtTimestamp возвращает версию документа на указанный момент времени
...
// CreateSnapshot создаёт снапшот всех версий для миграции
func CreateSnapshot(database, collection string) (map[string][]*DocumentVersion, error) {
    ...
    for _, doc := range docs {
        ...
        if len(versions) > 0 {
            ...
            // Если нет версий в MVCC, создаём текущую
            version := &DocumentVersion{
                Document:  doc.Clone(),
                Timestamp: time.Now().UnixMilli(),
...
// ApplySnapshot применяет снапшот к коллекции
func ApplySnapshot(database, collection string, snapshot map[string][]*DocumentVersion) error {
    ...
    for docID, versions := range snapshot {
        ...
        // Восстанавливаем все версии в MVCC
        if globalTxManager != nil {
            for _, v := range versions {
