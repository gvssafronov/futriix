...
// Назначение: Реализация транзакций с поддержкой MVCC и WAL без блокировок.
// Для распределённых транзакций используется протокол SAGA.
// ИСПРАВЛЕНО: Deadlock в flushBatch/rotateSegment (wm.mu рекурсивно)
// ИСПРАВЛЕНО: Race между async Write и Sync - добавлен механизм подтверждения
// ИСПРАВЛЕНО: Apply до WAL - теперь сначала WAL, потом применение
// ИСПРАВЛЕНО: Stop теряет записи буфера - flush перед закрытием
//
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ (паника atomic.Value):
//   Ранее currentTx был atomic.Value, и в CommitCurrentTransaction /
//   AbortCurrentTransaction вызывалось currentTx.Store(nil), что приводит
//   к панике "sync/atomic: store of nil value into Value".
//   Теперь currentTx хранит *txHolder (не-nil), внутри которого лежит
//   *Transaction (может быть nil = нет активной транзакции).
//   Использование helper-функций setCurrentTx/clearCurrentTx/getCurrentTx
//   полностью устраняет панику и совместимо с Go < 1.19 (OpenIndiana).
...
type MVCCStats struct {
    ...
    TotalSkippedPrune     atomic.Uint64
}
...
func NewMVCCManager(maxVersionsPerDoc int, retentionDays int) *MVCCManager {
    ...
    m.oldestReadTx.Store(0)
    go m.pruneOldVersionsLoop()
    return m
}

func (m *MVCCManager) RegisterReader(txID uint64) {
    m.readersMu.Lock()
    defer m.readersMu.Unlock()
    m.activeReaders.Store(txID, time.Now().UnixMilli())
    m.updateOldestReader()
}

func (m *MVCCManager) UnregisterReader(txID uint64) {
    m.readersMu.Lock()
    defer m.readersMu.Unlock()
    m.activeReaders.Delete(txID)
    m.updateOldestReader()
}

func (m *MVCCManager) updateOldestReader() {
    oldest := int64(^uint64(0) >> 1)
    m.activeReaders.Range(func(key, value interface{}) bool {
        if ts, ok := value.(int64); ok && ts < oldest {
            oldest = ts
        }
        return true
    })
    m.oldestReadTx.Store(oldest)
}
...
func (m *MVCCManager) CreateVersion(doc *Document, txID TransactionID) *DocumentVersion {
    ...
    if len(versions) > m.maxVersionsPerDoc {
        oldestReader := m.oldestReadTx.Load()
        newVersions := make([]*DocumentVersion, 0, m.maxVersionsPerDoc)
        startIdx := len(versions) - m.maxVersionsPerDoc
        if startIdx < 0 {
            startIdx = 0
        }
        for i := startIdx; i < len(versions); i++ {
            newVersions = append(newVersions, versions[i])
        }
        if oldestReader > 0 {
            for i := 0; i < startIdx; i++ {
                if versions[i].Timestamp >= oldestReader {
                    newVersions = append([]*DocumentVersion{versions[i]}, newVersions...)
                }
            }
        }
        prunedCount := len(versions) - len(newVersions)
        versions = newVersions
        m.stats.TotalVersionsPruned.Add(uint64(prunedCount))
    }
...
func (m *MVCCManager) PruneOldVersions() {
    ...
    m.versions.Range(func(key, value interface{}) bool {
        ...
        for _, v := range versions {
            if v.Timestamp >= cutoffTime || (oldestReader > 0 && v.Timestamp >= oldestReader) {
...
func (vm *VisibilityMap) IsVisible(docID string, version uint64) bool {
    ...
    vm.entries.Range(func(k, v interface{}) bool {
        ...
        if entry.DocID == docID && version >= entry.VisibleFrom && version <= entry.VisibleTo && entry.IsVisible {
            found = true
            return false
        }
        return true
...
func (rtc *ReadTimestampCache) Set(docID string, timestamp int64, doc *Document) {
    ...
    rtc.cache.Store(key, &cachedEntry{doc: doc, cachedAt: time.Now(), accessCount: 0})
    rtc.size.Add(1)
}
...
func (o *SagaOrchestrator) leaderElectionLoop() {
    defer o.wg.Done()
    ticker := time.NewTicker(o.config.GetLeaderElectionInterval())
    defer ticker.Stop()
    for {
        select {
        case <-ticker.C:
...
func (o *SagaOrchestrator) electLeader() {
    ...
    leader := candidates[0]
    o.leaderID = leader
    isLeader := leader == o.coordinators[0].id
...
func (o *SagaOrchestrator) cleanupOrphanSteps() {
    ...
    if err != nil {
        if o.logger != nil {
            o.logger.Error(fmt.Sprintf("Failed to list pending sagas for cleanup: %v", err))
        }
        return
    }
    ...
    for _, state := range pending {
        if state.Status == "running" || state.Status == "compensating" {
            ...
            if elapsed > SagaOrphanTimeout.Milliseconds() {
                if o.logger != nil {
                    o.logger.Warn(fmt.Sprintf("Found orphan saga %s (last updated %d ms ago), cleaning up", state.ID, elapsed))
                }
                state.Status = "orphaned"
                state.LastError = fmt.Sprintf("orphaned after %d ms of inactivity", elapsed)
...
func (o *SagaOrchestrator) metricsLoop() {
    defer o.wg.Done()
    ticker := time.NewTicker(o.config.GetMetricsInterval())
    defer ticker.Stop()
    for {
        select {
        case <-ticker.C:
...
func (o *SagaOrchestrator) recordDuration(durationMs int64) {
    ...
    avg := o.metrics.TotalDurationMs.Load() / maxUint64(1, o.metrics.TotalCompleted.Load())
...
func (sm *SagaManager) cleanupExecutedOpsLoop() {
    ...
    defer ticker.Stop()
    cutoff := int64(7 * 24 * 3600 * 1000)
    for {
        select {
        case <-ticker.C:
            now := time.Now().UnixMilli()
            sm.executedOps.Range(func(key, value interface{}) bool {
                if ts, ok := value.(int64); ok && now-ts > cutoff {
...
func (sm *SagaManager) GetOrchestrator() *SagaOrchestrator { return sm.orchestrator }
...
// =============================================================================
// WAL MANAGER (не-сегментированный)
// =============================================================================
...
func (wm *WALManager) flushBatch(batch []*WALRecord) {
    ...
    for _, record := range batch {
        ...
        record.Length = uint32(len(data))
        dataWithCRC, err := json.Marshal(record)
        if err != nil {
            continue
        }
        header := make([]byte, 8)
        binary.BigEndian.PutUint32(header[0:4], uint32(len(dataWithCRC)))
        binary.BigEndian.PutUint32(header[4:8], record.CRC)
        if _, err := wm.writer.Write(header); err != nil {
            continue
        }
        if _, err := wm.writer.Write(dataWithCRC); err != nil {
            continue
        }
...
// Close - ИСПРАВЛЕНО: flush перед закрытием
...
type SegmentedWALManager struct {
    ...
    writeChan        chan *WALRecord
    stopCh           chan struct{}
    ackChan          chan uint64 // ИСПРАВЛЕНО: канал для подтверждения записи
    wg               sync.WaitGroup
    batchSize        int
    logger           LoggerInterface
...
func NewSegmentedWALManager(segmentsDir string, fsyncEnabled bool, logger LoggerInterface) (*SegmentedWALManager, error) {
    ...
    wm := &SegmentedWALManager{
        ...
        ackChan:      make(chan uint64, 10000),
        batchSize:    100,
        logger:       logger,
        fsyncEnabled: fsyncEnabled,
    }
    ...
    if err := wm.validateSegments(); err != nil {
        if logger != nil {
            logger.Warn(fmt.Sprintf("WAL segment validation failed: %v", err))
        }
    }
...
func (wm *SegmentedWALManager) validateSegments() error {
    wm.mu.Lock()
    defer wm.mu.Unlock()
    for id, segment := range wm.segments {
        if segment.File == nil {
            file, err := os.OpenFile(segment.Path, os.O_RDWR, 0644)
            if err != nil {
                if wm.logger != nil {
                    wm.logger.Warn(fmt.Sprintf("WAL segment %d file missing: %s", id, segment.Path))
                }
                delete(wm.segments, id)
                continue
            }
            segment.File = file
            segment.Writer = bufio.NewWriterSize(file, 64*1024)
        }
    }
    return nil
}

func (wm *SegmentedWALManager) GetBackupLSN() uint64    { return wm.backupLSN.Load() }
func (wm *SegmentedWALManager) SetBackupLSN(lsn uint64) { wm.backupLSN.Store(lsn) }
...
// rotateSegmentInternal - ИСПРАВЛЕНО: без захвата wm.mu (вызывается из flushBatch с уже захваченным mu)
func (wm *SegmentedWALManager) rotateSegmentInternal() error {
    ...
    if wm.currentSegment != nil {
        if err := wm.currentSegment.Writer.Flush(); err != nil {
            if wm.logger != nil {
                wm.logger.Error(fmt.Sprintf("Failed to flush old segment: %v", err))
            }
        }
        if wm.fsyncEnabled {
            if err := RealFsyncWithRetry(wm.currentSegment.File, FsyncMaxRetries, FsyncRetryDelay); err != nil {
                if wm.logger != nil {
                    wm.logger.Error(fmt.Sprintf("Failed to fsync old segment: %v", err))
                }
            }
        }
...
// rotateSegment - публичный метод (захватывает mu)
func (wm *SegmentedWALManager) rotateSegment() error {
    wm.mu.Lock()
    defer wm.mu.Unlock()
    return wm.rotateSegmentInternal()
}
...
// WriteSync - ИСПРАВЛЕНО: синхронная запись с подтверждением через ackChan
func (wm *SegmentedWALManager) WriteSync(record *WALRecord) error {
    record.Timestamp = time.Now().UnixMilli()
    wm.writeChan <- record

    // Ждём подтверждения записи (по LSN или таймауту)
    select {
    case <-wm.ackChan:
        return nil
    case <-time.After(5 * time.Second):
        return fmt.Errorf("WAL write sync timeout")
    }
}
...
func (wm *SegmentedWALManager) writerLoop() {
    ...
    for {
        ...
        case <-wm.stopCh:
            // ИСПРАВЛЕНО: flush оставшихся записей перед выходом
            if len(batch) > 0 {
                wm.flushBatch(batch)
            }
            return
...
// flushBatch - ИСПРАВЛЕНО: не освобождает wm.mu при ротации (вызывает rotateSegmentInternal)
func (wm *SegmentedWALManager) flushBatch(batch []*WALRecord) {
    wm.mu.Lock()
    defer wm.mu.Unlock()
    for _, record := range batch {
        if wm.currentSegment.Size >= WALSegmentSize {
            // ИСПРАВЛЕНО: вызываем внутренний метод без повторного захвата mu
            if err := wm.rotateSegmentInternal(); err != nil {
                if wm.logger != nil {
    ...
    // ИСПРАВЛЕНО: отправляем подтверждение для всех записей в batch
    for range batch {
        select {
        case wm.ackChan <- 1:
        default:
...
// Close - ИСПРАВЛЕНО: flush оставшихся записей перед закрытием
...
// Stop - ИСПРАВЛЕНО: ждём обработки буфера перед закрытием
func (arm *AsyncRecoveryManager) Stop() {
    ...
    // ИСПРАВЛЕНО: ждём обработки оставшихся записей
    close(arm.recordChan)
    arm.wg.Wait()
    close(arm.errChan)
}
...
func (dd *DeadlockDetector) SetLogger(logger LoggerInterface) { dd.logger = logger }
...
// txHolder оборачивает указатель на транзакцию.
//
// КРИТИЧЕСКОЕ ИСПРАВЛЕНИЕ:
// atomic.Value не допускает сохранения nil — это вызывает панику
// "sync/atomic: store of nil value into Value". Поэтому мы всегда
// храним не-nil указатель на txHolder, а «нет активной транзакции»
// обозначается txHolder{tx: nil}.
...
var (
    ...
    // currentTx — всегда хранит *txHolder (никогда не nil).
    // Инициализируется в init() ниже.
...
func init() {
    // Инициализируем atomic.Value типизированным значением,
    // чтобы Load() всегда возвращал *txHolder и не падал при
    // приведении типа в getCurrentTx().
    currentTx.Store(&txHolder{tx: nil})
}

// setCurrentTx сохраняет указатель на активную транзакцию.
func setCurrentTx(tx *Transaction) {
    currentTx.Store(&txHolder{tx: tx})
}

// clearCurrentTx очищает текущую транзакцию (безопасно для atomic.Value).
func clearCurrentTx() {
    currentTx.Store(&txHolder{tx: nil})
}

// getCurrentTx возвращает указатель на текущую транзакцию или nil.
func getCurrentTx() *Transaction {
    val := currentTx.Load()
    if val == nil {
        return nil
    }
    holder, ok := val.(*txHolder)
    if !ok || holder == nil {
        return nil
    }
    return holder.tx
...
func GetTransactionManager() *TransactionManager { return globalTxManager }
...
func BeginTransactionWithOptions(options *TransactionOptions) *Transaction {
    ...
    if options == nil {
        options = &TransactionOptions{Timeout: DefaultTxTimeout}
    }
    tx := &Transaction{
...
func BeginTransactionWithTimeout(timeout time.Duration) *Transaction {
    return BeginTransactionWithOptions(&TransactionOptions{Timeout: timeout})
}
...
// CommitCurrentTransaction - ИСПРАВЛЕНО: сначала WAL, потом применение
func CommitCurrentTransaction() error {
    ...
    // ИСПРАВЛЕНО: Сначала записываем транзакцию в WAL (prepared state)
    ...
    walRecord := &WALRecord{Type: 1, Data: data}
    // ИСПРАВЛЕНО: используем WriteSync для гарантии записи в WAL до применения
    if err := globalTxManager.wal.WriteSync(walRecord); err != nil {
        return fmt.Errorf("failed to write transaction to WAL: %v", err)
    }

    // Теперь применяем операции
...
func CommitDistributedTransaction(txID TransactionID) error {
    ...
    if err := globalTxManager.wal.WriteSync(&WALRecord{Type: 1, Data: data}); err != nil {
...
func applyOperation(op Operation) error {
    ...
    case "restore":
        return coll.RestoreDeleted(op.DocumentID)
    }
    return nil
}
...
// createCheckpoint - ИСПРАВЛЕНО: fsync для чекпоинта
func (tm *TransactionManager) createCheckpoint() {
    ...
    // ИСПРАВЛЕНО: используем writeFileSync с fsync
    if err := writeFileSync(checkpointPath, data, 0644); err != nil {
        if tm.logger != nil {
...
func (tm *TransactionManager) IsRecoveryComplete() bool { return tm.recoveryComplete.Load() }
...
func (tm *TransactionManager) IsBackupInProgress() bool { return tm.backupInProgress.Load() }

func GetTransactionStats() map[string]interface{} {
    if globalTxManager == nil {
        return map[string]interface{}{"error": "transaction manager not initialized"}
    }
    ...
    return map[string]interface{}{
        ...
        "total_timed_out":      stats.TotalTimedOut.Load(),
        "total_deadlocks":      stats.TotalDeadlocks.Load(),
        "active_count":         active,
...
func HasActiveTransaction() bool { return getCurrentTx() != nil }
...
func MVCCSnapshot() uint64 { return uint64(time.Now().UnixNano()) }
...
func GetDocumentAtTimestamp(docID string, timestamp int64) (*Document, error) {
    ...
    doc := globalTxManager.GetDocumentVersion(docID, timestamp)
    if doc == nil {
...
func GetDocumentVersionsSince(docID string, since int64) ([]*DocumentVersion, error) {
    if globalTxManager == nil {
        return nil, fmt.Errorf("transaction manager not initialized")
    }
    val, ok := globalTxManager.documentVersions.Load(docID)
    if !ok {
        return nil, fmt.Errorf("no versions found for document %s", docID)
    }
    versions := val.([]*DocumentVersion)
    result := make([]*DocumentVersion, 0)
    for _, v := range versions {
...
func CreateSnapshot(database, collection string) (map[string][]*DocumentVersion, error) {
    if globalStorage == nil {
        return nil, fmt.Errorf("storage not initialized")
    }
    db, err := globalStorage.GetDatabase(database)
    if err != nil {
        return nil, err
    }
    ...
    result := make(map[string][]*DocumentVersion)
    for _, doc := range docs {
        versions, err := GetDocumentVersionsSince(doc.ID, 0)
        if err != nil {
            continue
        }
        if len(versions) > 0 {
            result[doc.ID] = versions
        } else if globalTxManager != nil {
            version := &DocumentVersion{
                Document:  doc.Clone(),
                Timestamp: time.Now().UnixMilli(),
                TxID:      TransactionID(0),
                VersionID: fmt.Sprintf("%s_%d", doc.ID, time.Now().UnixNano()),
            }
            globalTxManager.AddDocumentVersion(doc.ID, version)
            result[doc.ID] = []*DocumentVersion{version}
        }
    }
    return result, nil
}

func ApplySnapshot(database, collection string, snapshot map[string][]*DocumentVersion) error {
    if globalStorage == nil {
        return fmt.Errorf("storage not initialized")
    }
    db, err := globalStorage.GetDatabase(database)
    ...
    for docID, versions := range snapshot {
        if len(versions) == 0 {
            continue
        }
        latest := versions[len(versions)-1]
        doc := latest.Document
        existing, err := coll.Find(docID)
        if err == nil && existing != nil {
            updates := doc.GetFields()
            if err := coll.Update(docID, updates); err != nil {
                return fmt.Errorf("failed to update document %s: %v", docID, err)
            }
        } else {
            if err := coll.Insert(doc); err != nil {
                return fmt.Errorf("failed to insert document %s: %v", docID, err)
            }
        }
        if globalTxManager != nil {
            for _, v := range versions {
                globalTxManager.AddDocumentVersion(docID, v)
            }
        }
    }
    return nil
...
func GetMVCCManager() *MVCCManager {
    if globalTxManager == nil {
        return nil
    }
    return globalTxManager.mvccManager
}

func GetMVCCStats() map[string]interface{} {
    if globalTxManager == nil || globalTxManager.mvccManager == nil {
        return map[string]interface{}{"error": "MVCC manager not initialized"}
    }
    return globalTxManager.mvccManager.GetMVCCStats()
}

func GetVisibilityMapStats() map[string]interface{} {
    if globalTxManager == nil || globalTxManager.visibilityMap == nil {
        return map[string]interface{}{"error": "Visibility map not initialized"}
    }
    return globalTxManager.visibilityMap.GetStats()
}

func GetReadCacheStats() map[string]interface{} {
    if globalTxManager == nil || globalTxManager.readCache == nil {
        return map[string]interface{}{"error": "Read cache not initialized"}
    }
    return globalTxManager.readCache.GetStats()
}
