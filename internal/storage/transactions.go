...
func NewSagaOrchestrator(baseDir string, nodeID string, logger LoggerInterface) (*SagaOrchestrator, error) {
    storage, err := NewSagaPersistentStorage(baseDir, logger)
    if err != nil {
        return nil, err
    }
    ...
    // Запускаем координаторы (несколько для отказоустойчивости)
    for i := 0; i < 3; i++ {
    ...
    // Запускаем лидер-элекшн
    o.wg.Add(1)
    go o.leaderElectionLoop()

    // Запускаем восстановление незавершённых SAGA
    o.wg.Add(1)
    go o.recoveryLoop()
    ...
    if logger != nil {
        logger.Info(fmt.Sprintf("Saga orchestrator initialized on node %s with %d coordinators", nodeID, len(o.coordinators)))
    }
        ...
        case <-ticker.C:
            ...
            c.activeSagas.Range(func(key, value interface{}) bool {
                ...
                if saga.Status == "running" {
                    ...
                    if time.Since(time.UnixMilli(saga.UpdatedAt)) > 5*time.Minute {
