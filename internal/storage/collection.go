...
type Collection struct {
    ...
    // ИСПРАВЛЕНО: Блокировка для согласованности индексов при SoftDelete
    indexMu     sync.Mutex            // Защита атомарности операций с индексами
}
...
func (c *Collection) Update(id string, updates map[string]interface{}) error {
    ...
    // ИСПРАВЛЕНО: Блокируем индексы для атомарности
    c.indexMu.Lock()
...
// ИСПРАВЛЕНО: Атомарность SoftDelete и удаления из индексов
func (c *Collection) Delete(id string) error {
    ...
    if c.metadata.Settings.SoftDelete {
        // ИСПРАВЛЕНО: Блокируем индексы для атомарности SoftDelete и удаления из индексов
        c.indexMu.Lock()
        ...
        // ИСПРАВЛЕНО: Блокируем индексы для атомарности физического удаления
        c.indexMu.Lock()
...
// ИСПРАВЛЕНО: Атомарность восстановления и добавления в индексы
