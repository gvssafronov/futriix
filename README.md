...
> [!CAUTION]
> **ALPHA VERSION**<br><br>**Проект достаточно стабилен в тестовых сценариях, но категорически не рекомендуется для промышленного использования до выхода версии 3.0.**<br>
...
По своей сути futriix является лёгкой HTAP‑СУБД с уклоном в OLTP благодаря lock‑free и wait‑free структурам и ACID‑транзакциям, а также мощными OLAP‑возможностями через индексы, триггеры, Lua‑плагины и аналитические функции. Это позволяет обрабатывать и анализировать данные в режиме, близком к реальному времени, в первую очередь для эксплуатации в замкнутых программных средах под управлением ОС на базе ядра Illumos и Solaris (OpenIndiana Hipster, Oracle Solaris), а кроме того, она совместима с популярными Linux‑дистрибутивами (Debian, Ubuntu, Fedora).

Архитектурное решение использовать отдельные индексы от данных, атомарные операции и версионирование позволяет эффективно обрабатывать смешанную нагрузку без необходимости применять отдельные системы для OLTP и OLAP. В futriix реализован WUI‑интерфейс для удобства её администрирования и эксплуатации. <br>

Проект futriix — это MongoDB‑совместимая СУБД без блокировок, разрабатываемая в экосистеме OpenIndiana и распространяемая под лицензией CDDL. Проект реализует полный набор требований к надёжной распределённой системе: WAL (write‑ahead logging) для устойчивости к сбоям, MVCC (multi‑version concurrency control) для изоляции транзакций, шардинг для горизонтального масштабирования и ACID‑гарантии на уровне транзакций. Ключевая особенность — построение ядра на неблокирующих структурах данных и консенсусе Raft, что обеспечивает предсказуемую производительность в высоконагруженных сценариях. <br>

Временные метки (timestamps) — это значения, фиксирующие момент времени, к которому относится событие или версия данных. В контексте futriix они используются для: <br>

* Версионирования записей: каждая модификация данных получает уникальную временную метку, что позволяет отличать старые и новые версии и корректно разрешать конфликты при репликации;
* Обеспечения MVCC: временные метки служат основой для определения видимости данных в рамках изоляции транзакций — транзакция видит только те версии записей, которые актуальны на момент её начала;
* Упорядочивания событий в распределённой среде: метки позволяют выстроить хронологию операций между узлами кластера, даже если локальные часы узлов не синхронизированы идеально;
* Реализации политик хранения и TTL: временные метки дают возможность автоматически удалять или архивировать устаревшие данные по истечении заданного срока жизни;
* Аудита и отладки: метки фиксируют точное время возникновения событий (запись, чтение, ошибка), что упрощает диагностику и восстановление состояния системы после сбоев. <br>
...
> [!CAUTION]
> **СУБД работает только на Unix‑подобных ОС. Поддержка Windows и macOS не предусмотрена!**<br>
...
Плагины имеют доступ к БД, транзакциям, триггерам, могут логировать события и взаимодействовать через событийную шину, а также они доступны в веб-интерфейсе. <br>
