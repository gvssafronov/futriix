...
> Дисковая подсистема: SSD/NVMe; HDD допустим только для тестов <br>
> Файловая система: предпочтительно ZFS (особенно на Illumos) или XFS/ext4 на Linux<br>
> Сеть: стабильное соединение между узлами, желательно 1 Gbps+; для Raft‑кластеров важна низкая задержка (latency)<br>
> Время: синхронизация времени между узлами (NTP/Chrony) обязательна для корректной работы MVCC, WAL и транзакций <br>
