...
Есть WUI‑интерфейс для администрирования. Проект развивается в экосистеме OpenIndiana, распространяется под лицензией CDDL и совместим с ОС на базе Illumos/Solaris (OpenIndiana Hipster, Oracle Solaris), а также с популярными Linux‑дистрибутивами (Debian, Ubuntu, Fedora).
...
**Вопрос:** Почему версия обозначена как «Futriix 2 i²», а не просто «2.0»?
<br>
**Ответ:** «2» — номер версии: здесь собраны ключевые компоненты (шардинг, WAL, MVCC и др.).
i2i2 — математическая метафора: хотя ii — мнимая единица, её квадрат даёт абсолютно точный результат (i2=−1i2=−1). Так и Futriix: даже в сложной распределённой среде с параллельными операциями (wait‑free, lock‑free) система выдаёт строго предсказуемые результаты — за счёт Raft, ACID и MVCC.
Это близко философии OpenIndiana/Illumos: детерминизм и контроль над ресурсами.
